...
int SYM_ARROW        = 33; //REFERENCE SYMBOL FOR THE STRUCT
int SYM_AND          = 34; // &&
int SYM_OR           = 35; // ||
int SYM_NOT          = 36; // !
...
void createSymbolTableEntry(int which, int* string, int line, int class, int type, int value, int address, int array1dLength, int array2dLength);
...
int  set1dArrayLength(int* entry, int array1dLength){*(entry + 8) = array1dLength; }
int  set2dArrayLength(int* entry, int array2dLength){*(entry + 9) = array2dLength; }
...
void resetAttribute(int * attribute){
    *(attribute) = ATT_NOT;
    *(attribute + 1) = 0;
}
...
void createSymbolTableEntry(int whichTable, int* string, int line, int class, int type, int value, int address, int array1dLength, int array2dLength) {
    int* newEntry;
    ...
    set1dArrayLength(newEntry, array1dLength);
    set2dArrayLength(newEntry, array2dLength);
...
//helping method to make the code smaller
...
//helping method to make the code smaller
int checkDereferenceForFactor(int type, int* attribute ){
    getSymbol();
    // ["*"] identifier
    if (symbol == SYM_IDENTIFIER) {
...
int gr_factor(int* attribute) {
    ...
    // dereference
    if (symbol == SYM_ASTERISK) {
        type = checkDereferenceForFactor(type, attribute );
        // identifier
    } else if (symbol == SYM_IDENTIFIER) {
        variableOrProcedureName = identifier;
        getSymbol();
        //identifier ("(" | "[" )
        type = checkSymbolAfterIdentifier(type,variableOrProcedureName, entry);
        // integer?
    } else if (symbol == SYM_INTEGER) {
...
////helping method for emitting the code for operatorSymbol to make the code smaller
void emitCodeForOperator(int operatorSymbol, int ltype, int rtype){
    // term operator symbols ( "*" | "/" | "%" )
    if (operatorSymbol == SYM_ASTERISK) { // "*"
        ...
        // simple expression operator symbols ( "+" | "-" )
    } else if (operatorSymbol == SYM_MOD) {// "%"
        emitRFormat(OP_SPECIAL, previousTemporary(), currentTemporary(), 0, FCT_DIVU);
        emitRFormat(OP_SPECIAL, 0, 0, previousTemporary(), FCT_MFHI);
        tfree(1);

    }else if (operatorSymbol == SYM_PLUS) {// "+"
        if (ltype == INTSTAR_T) {
            if (rtype == INT_T){  emitLeftShiftBy(2);  }
        } else if (rtype == INTSTAR_T){  typeWarning(ltype, rtype);  }
        emitRFormat(OP_SPECIAL, previousTemporary(), currentTemporary(), previousTemporary(), FCT_ADDU);
        tfree(1);

    } else if (operatorSymbol == SYM_MINUS) {// "-"
        if (ltype != rtype){  typeWarning(ltype, rtype);  }
        emitRFormat(OP_SPECIAL, previousTemporary(), currentTemporary(), previousTemporary(), FCT_SUBU);
        tfree(1);

        //shift expression operator symbols ( "<<" | ">>"  )
    }else if (operatorSymbol == SYM_LEFT_SHIFT) {// "<<"
    ...
    } else if(operatorSymbol == SYM_EQUALITY){// ==
        emitRFormat(OP_SPECIAL, previousTemporary(), currentTemporary(), previousTemporary(), FCT_SUBU);
        tfree(1);
...
//helping method for constant folding to make the code smaller
            ...
            if (symbol == SYM_IDENTIFIER) {
                ...
                if (symbol == SYM_LPARENTHESIS){
                    ...
                    if (isLiteral()){
                        ...
                        allocatedMemory = allocatedMemory + (size * WORDSIZE);
                        createSymbolTableEntry(GLOBAL_TABLE, variableOrProcedureName, lineNumber, VARIABLE, type, 0, -allocatedMemory, array1dLength, array2dLength);
                        checkSymbolAndGetNew(SYM_SEMICOLON);
                    }else{
...
void emitGlobalsStrings() {
    ...
    while ((int) entry != 0) {
        if (getClass(entry) == VARIABLE) {
            ...
            binaryLength = binaryLength + get1dArrayLength(entry) * get2dArrayLength(entry)* WORDSIZE;
